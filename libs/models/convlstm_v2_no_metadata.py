"""
    Same as ConvLSTM model v2 but without metadata

    This model underperforms the variant with metadata.
...
def my_conv_lstm_model_builder(
        ...
        verbose=True):
    """
        Builder function for the second convlstm model without metadata.
    ...
    def my_convlstm_model(my_cnn_encoder, my_head):
        ...
        return tf.keras.Model([img_input, metadata_input], x, name='convLSTMModel_v2_no_metadata')
    ...
    my_head = my_head(input_size=my_cnn_encoder.layers[-1].output_shape[1],
